...
CallbackReturn VescHwInterface::on_init(const hardware_interface::HardwareInfo& info)
{
  ...
  {
    RCLCPP_FATAL(rclcpp::get_logger("VescHwInterface"), "Joint '%s' has %zu command interfaces found. 1 expected.",
  ...
  {
    RCLCPP_FATAL(rclcpp::get_logger("VescHwInterface"), "Joint '%s' has %zu state interface. 2 expected.",
...
void VescHwInterface::packetCallback(const std::shared_ptr<VescPacket const>& packet)
{
  ...
  else if (packet->name() == "Values")
  {
    ...
    const double steps = values->tachometer();
